...
#' @export
ClosestDiagonalAgriCell <- function(matrix, agriCatList, targetPosMatrix, swfCat, agriCat, ExpPriority="mixed", ExpDirection, numCells, maxGDistance, np) {
    nrows <- nrow(matrix)
    ncols <- ncol(matrix)
    ...
    parallel::mclapply(1:nrow(targetPosMatrix), function(index) {
        ...
        while(nrow(selectedNeighbors) < numCells && radius <= maxRadius) {
            ...
            # Filter agriCat cells within the current radius
        ...
        orderedNeighbors <- orderCellsByDistance(selectedNeighbors, r, c, ExpPriority, ExpDirection)
