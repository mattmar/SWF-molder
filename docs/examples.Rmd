...
swf.raster <- rast('/home/matteo/own_data/PoD/jrc/data/subset/clc_swf_10m.tif')
swt.mt <- t(as.matrix(swf.raster, wide=T))
storage.mode(swt.mt) <- "integer"
```

## Run function
```{r,  message=FALSE, warning=FALSE}
iters <- 50
smf.L <- swf.clumper.regular(Hmatrix=swt.mt, swfCat=1, agriCat=2, foreCat=3, sigma=2, Q=5, iterations=iters, kernelCl=100, kernelRw=100, NNeighbors=3, queensCase=TRUE, np=7, maxDistance = 2, Density="L", deBug=FALSE)
```

## Plot results
```{r,  message=FALSE, warning=FALSE}
diff <- smf.L[[length(smf.L)]] - swt.mt
image(diff)
```

```{r,  message=FALSE, warning=FALSE, hide=TRUE}
layout_matrix <- matrix(c(1, 2, 3), ncol = 1, byrow = TRUE)
trellis.par.set(layout.heights = list(top.padding = 1, main.key.padding = 1.5, key.axis.padding = -1, axis.xlab.padding = -1, bottom.padding = -1))
trellis.par.set(layout.widths = list(left.padding = -1, key.ylab.padding = -1, right.padding = -1))
```

```{r, fig.width=8, fig.height=15, fig.align='center'}
print(levelplot(swt.mt, colorkey=FALSE, xlab="", ylab="", main="Original", scales=list(x=list(draw=FALSE), y=list(draw=FALSE))),  split=c(1, 1, 1, 3), more=TRUE) 
print(levelplot(smf.L[[length(smf.L)]], colorkey=FALSE, xlab="", ylab="", main=paste("Rearranged after", iters, "iterations"), scales=list(x=list(draw=FALSE), y=list(draw=FALSE))),  split=c(1, 2, 1, 3), more=TRUE)
print(levelplot(diff, colorkey=FALSE, xlab="", ylab="", main="Difference", scales=list(x=list(draw=FALSE), y=list(draw=FALSE))), 
